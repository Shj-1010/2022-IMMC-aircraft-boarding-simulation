...
# 페르소나별 시각화 색상 매핑
PERSONA_COLORS = {
    'window': 'cyan',        # 창가 선호족: 하늘색
    'aisle': 'pink',         # 통로 선호족: 분홍색
    'middle': 'lightgreen',  # 중간 선호족: 연두색
    'apathetic': 'dimgray'   # 무신경족: 짙은 회색
}
...
# ==========================================
class Passenger:
    def __init__(self, p_id, target_row, target_seat):
        self.id = p_id
        ...
        if BOARDING_STRATEGY not in ['ROW', 'FREE']:
            if random.random() < MISS_SEAT_PROBABILITY:
                if BOARDING_STRATEGY in ['RANDOM', 'WILMA']:
...
# --- [수정됨: 자유좌석제 페르소나 및 빈자리 가중치 로직 적용] ---
...
# 4. 초기화 및 수납 로직 함수
# ==========================================
passengers = [Passenger(i, r, s) for i, (r, s) in enumerate((r, s) for r in range(NUM_ROWS) for s in range(SEATS_PER_ROW))]

if BOARDING_STRATEGY == 'RANDOM': boarding_queue = get_random_queue(passengers)
elif BOARDING_STRATEGY == 'ZONE': boarding_queue = get_zone_queue(passengers)
elif BOARDING_STRATEGY == 'WILMA': boarding_queue = get_wilma_queue(passengers)
elif BOARDING_STRATEGY == 'ROW': boarding_queue = get_row_queue(passengers)
elif BOARDING_STRATEGY == 'FREE': boarding_queue = get_free_seating_queue(passengers)

aisle_passengers = [] 
seating_passengers = [] 
# --- [수정됨: 문자열(persona)을 담을 수 있도록 dtype=object 로 변경] ---
...
def update(frame):
    ...
    if all(p.state == 'seated' for p in passengers):
        ax.set_title(f"[{BOARDING_STRATEGY}] Boarding Complete! Total Time: {time_ticks} sec")
        return
    ...
    ax.clear()
    ax.set_xlim(-1, NUM_ROWS)
    ax.set_ylim(-1, 7)
    title_text = f"[{BOARDING_STRATEGY}] Time: {time_ticks} sec"
    if BOARDING_STRATEGY == 'FREE':
        title_text += " | Cyan:Win, Pink:Aisle, Grn:Mid, Gry:Any"
    ax.set_title(title_text)
    
    ax.set_yticks([0, 1, 2, 4, 5, 6])
    ax.set_yticklabels(['Window(L)', 'Middle(L)', 'Aisle(L)', 'Aisle(R)', 'Middle(R)', 'Window(R)'])
    ax.set_xlabel("Row Number")
    
    # --- [수정됨: 앉은 승객을 페르소나 색상으로 시각화] ---
    for r in range(NUM_ROWS):
        for s in range(SEATS_PER_ROW):
            y_pos = s if s < 3 else s + 1
            seat_val = seated_matrix[r, s]
            
            if seat_val == 0:
                color = 'lightgray'
            else:
                # FREE 모드일 경우 각자의 페르소나 색상, 그 외 전략은 파란색 통일
                color = PERSONA_COLORS.get(seat_val, 'blue') if BOARDING_STRATEGY == 'FREE' else 'blue'
                
            ax.plot(r, y_pos, 's', color=color, markersize=10)

    for p in seating_passengers:
        y_pos = p.target_seat if p.target_seat < 3 else p.target_seat + 1
        ax.plot(p.target_row, y_pos, 'o', color='green', markersize=8)

    for p in aisle_passengers:
        if p.missed_seat: color = 'purple'      
        elif p.returning: color = 'magenta'     
        elif p.state == 'walking': color = 'red' 
        else: color = 'orange'                   
            
        visual_x = p.pos_m / SEAT_PITCH 
        ax.plot(visual_x, 3, 'o', color=color, markersize=8)

ani = animation.FuncAnimation(fig, update, frames=1000, interval=50, repeat=False)
plt.show()